...
class CompilerGUI:
    def __init__(self, root):
        ...
        ttk.Button(steps_frame, text="3. C Detection", 
                  command=self.run_c_detection, style="Step.TButton").pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(steps_frame, text="4. Compile & Run", 
...
int main() {
    ...
    def run_c_detection(self):
        try:
            self.output_text.delete(1.0, tk.END)
            self.output_text.insert(tk.END, "Running C Code Detection...\n\n")
            
            code = self.code_input.get(1.0, tk.END)
            c_indicators = [
                ("#include <stdio.h>", "C-style header"),
                ("printf(", "C-style print function"),
                ("scanf(", "C-style input function"),
                ("malloc(", "C memory allocation"),
                ("free(", "C memory deallocation")
            ]
            
            findings = []
            for indicator, description in c_indicators:
                if indicator in code:
                    findings.append(f"✓ Found {description}: {indicator}")
            
            if findings:
                self.output_text.insert(tk.END, "🔍 C Code Detection Results:\n\n")
                for finding in findings:
                    self.output_text.insert(tk.END, f"{finding}\n")
                self.output_text.insert(tk.END, "\n⚠️ This code contains C-style elements that could be modernized in C++.")
                self.flag=1
                
            else:
                self.output_text.insert(tk.END, "✅ No C-style code patterns detected.\nThis appears to be modern C++ code.")
    ...
    def compile_and_run(self):
        if(self.flag==1):
             self.output_text.delete(1.0, tk.END)
             self.output_text.insert(tk.END, "❌ Cannot compile: Detected C-style code in the input. Please modernize the code to C++ before compiling.\n\n")
        else:
            try:
                self.output_text.delete(1.0, tk.END)
                ...
                # Compile
                compile_result = subprocess.run(["g++", temp_file, "-o", "program"], 
                                             capture_output=True, 
                                             text=True)
